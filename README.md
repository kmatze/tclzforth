...
Added features
   ...
   - FMAIN     this word will be started after image loading
   - fsave     save the actual workspace as image in zfnew.exe
   - FMAIN     zforth.exe start FMAIN after image was successful loaded (autostart)
...
First start of zForth should be:
   1. start zforth.exe
   2. zforth cli: 130 sys forth\core.zf           // load system core
   3.             130 sys forth\dict.zf           // load words for handling dictionary
   4.             words                           // show words in dictionary
   5.             save                            // save work space as image in file zforth.save
   6.             fsave                           // save workspace as a extended zforth system in file zfnew.exe
   7.             bye                             // end the zForth session

 Now you can start with zForth in diffrent ways:
 a)   zforth.exe zforth.save                      // start zForth system and load saved image zforth.save
 b)   zfnew.exe                                   // start extended zForth system

tclzforth you can compile with gcc. See the batch file compile_tclzforth.bat

To start tclforth follow the next steps:

start tcl/tk shell, inside do:
...
With the next steps you can create own execute zForth (no tcl system!):
